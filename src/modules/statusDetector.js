    ...
    const goalStatuses = await Promise.all(
      weeklyGoals.map(async (goal) => {
        const reactions = await getReactions(
          ...
          channelId,
          goal.messageTs,
          emojiMapping[goal.emoji].reaction
        );
...
async function getWeeklyGoals(slack, channelId) {
  try {
    ...
    console.log(
      `Retrieved ${result.messages.length} messages from the channel`
    );
    ...
    const startDate = new Date(parseInt(goalMessage.ts.split(".")[0]) * 1000)
      .toISOString()
      .split("T")[0];
...
async function getReactions(slack, channelId, messageTs, emoji) {
  try {
    ...
    const targetReaction = result.message.reactions.find(
